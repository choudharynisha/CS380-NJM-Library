    ...
    if(isset($_POST['submit'])) {
        ...
        if($user->num_rows < 1) {
            echo "<script>
                    ...
                    window.location.href = 'http://comet.cs.brynmawr.edu/~nchoudhary/CS380-Library-System/librarian_login.php';
                </script>";
        } else {
            $hashedpassword = $user->fetch_assoc()['password'];
            if(password_verify($password, $hashedpassword)) {
                echo "Valid user";
                header("Location: http://comet.cs.brynmawr.edu/~nchoudhary/CS380-Library-System/librarian_index.php");
            } else {
                echo "<script>
                    alert('Invalid username and/or password');
                    window.location.href = 'http://comet.cs.brynmawr.edu/~nchoudhary/CS380-Library-System/librarian_login.php';
